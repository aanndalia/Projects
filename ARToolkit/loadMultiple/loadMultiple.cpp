...
int multY;
int multZ;
//char winMessage[] = "You Win. Balls Remaining: ";
...
int main(int argc, char **argv)
{
	...
	std::cout << "\nEnter level you would like to play" <<std::endl;
	std::cin >> level;
...
static void mouseEvent(int button, int state, int x, int y)
{
	if(button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN && mainloopcount > 200 && sqrt(((-1*slingVelX)+SLINGOFFX)*((-1*slingVelX)+SLINGOFFX)+((-1*slingVelZ)+SLINGOFFY)*((-1*slingVelZ)+SLINGOFFY)+slingVelY*slingVelY)<RANGE)
	{
...
static void   keyEvent( unsigned char key, int x, int y)   
{
	...
	if(key=='f'){
		...
		//linear
		addVelocityBall(20,slingPosX,slingPosZ,-slingPosY,((-1*slingVelX)+SLINGOFFX),((-1*slingVelZ)+SLINGOFFY),slingVelY);
		fprintf(pFile, "vel = %lf\n", sqrt(((-1*slingVelX)+SLINGOFFX)*((-1*slingVelX)+SLINGOFFX)+((-1*slingVelZ)+SLINGOFFY)*((-1*slingVelZ)+SLINGOFFY)+slingVelY*slingVelY));
...
static void mainLoop(void)
{
	...
	if(pigsRemain == false)
	{
		printf("\n\nYou win with %d balls remaining!\n", 10-numVelBalls);
		keyEvent('r',0,0);
	}
	else if(numVelBalls>=10)
	{
		printf("\n\nYou lose! Out of balls.\n");
		keyEvent('r',0,0);
	}
...
static void multidraw( double trans1[3][4], double trans2[3][4], int mode )
{
	...
	if(objectAdded){
		...
		if( object[0].visible == 1 && sqrt(((-1*slingVelX)+SLINGOFFX)*((-1*slingVelX)+SLINGOFFX)+((-1*slingVelZ)+SLINGOFFY)*((-1*slingVelZ)+SLINGOFFY)+slingVelY*slingVelY)<RANGE)
		{
			glBegin(GL_LINE_STRIP);
		...
		glEnable (GL_BLEND);
		glTranslatef(SLINGOFFX, 0,SLINGOFFY);
		glBlendFunc (GL_SRC_ALPHA, GL_ONE);
		glColor3f(0,1,0);
		glutSolidSphere(RANGE,10,10);
		glTranslatef(SLINGOFFX, 0,SLINGOFFY);
		glDisable (GL_BLEND);
		glColor3f(1,1,1);
