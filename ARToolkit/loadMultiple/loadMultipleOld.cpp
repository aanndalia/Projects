...
static void   keyEvent( unsigned char key, int x, int y)   
{
	...
	if(key=='b'){
		objectAdded=1;
		hkReal offset=-1.0*numBalls;
		addHavokCube(2.0f,2.0f,2.0f,0,200.0f,0.0f);
		numBalls=numBalls+1;
	}
	if(key=='k'){
		if(ourCubeAdded == 0){
	...
	if(obj_id == 0){
		...
		*/
		...
		if(objectAdded){
			...
			//fprintf (pFile, "\nVector size = %d, vector 2 size = %d, numBalls = %f ",objVector.size(), shapeVector.size(), numBalls);
			...
			/*	for(int k=0;k<16;k++){
				...
				(*objVector)[i]->getTransform().get4x4ColumnMajor(*quat);
			    //fprintf(pFile, "\n%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", quat[0][0], quat[0][1], quat[0][2], quat[0][3], quat[1][0], quat[1][1], quat[1][2], quat[1][3], quat[2][0], quat[2][1], quat[2][2], quat[2][3], quat[3][0], quat[3][1], quat[3][2], quat[3][3]);
				hkReal temp;
				for(int i=0;i<4;i++){
					for(int j=i+1;j<4;j++){
						temp = quat[j][i];
				...
				if((*shapeVector)[i]==0){
					...
					//fprintf (pFile, "sphere %d", i);
				}
				else if((*shapeVector)[i]==1)
				{
					glutSolidCube(2.0);
					//fprintf (pFile, "cube %d", i);
				}
				glLoadMatrixd( gl_para );
			}
		}
...
void addHavokCube(hkReal dimX,hkReal dimY,hkReal dimZ, hkReal posX, hkReal posY, hkReal posZ){
	...
	hkpInertiaTensorComputer::computeBoxVolumeMassProperties(cube_dim,150.0f,massProperties);
