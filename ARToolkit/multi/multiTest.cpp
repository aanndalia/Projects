...
double stackTest[3][4] = {{1, 0, 0, 200},
						  {0, 1, .75, 0},
						  {0, -.75, 1, 0}
						  };

double stackTest2[3][4];
/*
						= {{1, 0, 0, -200},
						  {0, 1, 0, 0},
						  {0, 0, 1, 0}
...
*/
    ...
    for( i = 0; i < config->marker_num; i++ ) {
		//changed from >=, not sure why it was, behavior seems the same
...
static void draw( double trans1[3][4], double trans2[3][4], int mode )
{
    ...
    GLfloat   mat_ambient1[]    = {1.0, 1.0, 0.0, 1.0};
    GLfloat   mat_flash[]       = {0.0, 1.0, 1.0, 1.0};
    GLfloat   mat_flash1[]      = {1.0, 1.0, 0.0, 1.0};
	...
	for( j = 0; j < 3; j++ ) {
        for( i = 0; i < 4; i++ ) printf("%8.4f ", trans1[j][i]);
        printf("\n");
    }

	for( j = 0; j < 3; j++ ) {
        for( i = 0; i < 4; i++ ) printf("%8.4f ", trans2[j][i]);
        printf("\n");  
    }
	printf("\n\n");
	...
	argConvGlpara(stackTest, gl_para);
	glMultMatrixd( gl_para );
	arUtilMatInv(stackTest,stackTest2); //multiplying by inverse undoes shift of last multiply
	argConvGlpara(stackTest2, gl_para);
	glMultMatrixd( gl_para );
	//loading more to the stack offsets, what's up with that?
